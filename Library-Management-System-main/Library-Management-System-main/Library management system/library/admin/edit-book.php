...
$sql="update  books set BookName=:bookname,CatId=:category,AuthorId=:author,ISBNNumber=:isbn,BookPrice=:price where id=:bookid";
...
<?php 
$bookid=intval($_GET['bookid']);
$sql = "SELECT books.BookName,genre.CategoryName,genre.id as cid,authors.AuthorName,authors.id as athrid,books.ISBNNumber,books.BookPrice,books.id as bookid from  books join genre on genre.id=books.CatId join authors on authors.id=books.AuthorId where books.id=:bookid";
$query = $dbh -> prepare($sql);
$query->bindParam(':bookid',$bookid,PDO::PARAM_STR);
$query->execute();
...
<?php 
$status=1;
$sql1 = "SELECT * from  genre where Status=:status";
